...
当前账号密码登录首次建档创建 `USER/UNVERIFIED` 用户。普通用户需先请求后端随机签发的课堂演示码，前端通过网页顶部模拟短信通知展示，再将手机号、验证码、姓名、学号、院系和年级提交到 `POST /auth/campus-verification`；手机号与学号均不可重复绑定。成功后直接设为 `VERIFIED`，仍没有管理员人工审核环节。演示码会直接返回给当前登录用户，不能当作真实短信安全能力。
