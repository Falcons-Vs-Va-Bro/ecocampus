...
最近一次数据库配置、迁移与生产链路复核：2026-07-16。
...
- Flyway 当前为 V1–V6：核心表、4 个初始类目、账号密码哈希列、会话双方已读时间、独立绑定手机号，以及类目父子关系/启停状态。
- `db/seed/R__mysql_demo_seed.sql` 和 `R__mysql_catalog_seed.sql` 是 repeatable Flyway 演示 seed；后者使用预留 ID `50001`–`50072` 和 `60001`–`60016`，前一段为九类目各 8 件常规商品，后一段为明确标注非官方联名的校园抽象梗商品。生产 profile 会解析这两份已登记脚本，校验和不变时不会重复执行，脚本变更则会按 Flyway repeatable 语义再次运行。
...
6. 默认 MySQL 环境的自动 demo seed 有 9 个一级类目；后端测试库只执行结构 migration，因此只有 V2 的 4 个基础类目。
7. 类目已支持真实两级结构、启停和实时商品数；当前还没有拖拽排序，排序值通过编辑表单保存。
...
14. 后台用户页的总量、今日新增、注册日期和发布数仍有部分硬编码展示值；类目页已移除同类本地数据。
...
- 2026-07-16 首页抽象梗商品扩展后，catalog 共 128 个 WebP、来源清单 115 条映射；16 件梗商品已迁移到 `60001`–`60016`，对应 16 张图片和 19 条配送方式映射。首次部署曾错误复用真实自增商品 `50073`–`50075`：修复脚本从 `ITEM_CREATED` 审计恢复原卖家并将三条受损商品保护性下架，原始标题、描述、价格和图片关联因无快照无法自动重建，需原卖家重新编辑；仅由错误 seed 创建的 `50076`–`50088` 被标记为 `DELETED`。
...
- 2026-07-16 商品编辑、订单履约、个人资料和类目管理真实接线后，`cd frontend && pnpm lint && pnpm build`、`cd backend && ./mvnw test` 全部通过，后端共 35 项测试、0 failure/error/skip；针对类目、商品卖家、订单和用户资料的 9 项回归测试再次通过。GitHub Pages 工作流 `29489061397` 与 Mac mini 后端工作流 `29489061354` 均发布成功，公网 health 为 `UP`，生产 Flyway V6 执行成功。
- 2026-07-16 生产 E2E 使用真实学生与管理员会话完成：商品编辑保存后刷新仍保留、下架后进入 `OFF_SHELF`；昵称通过 `PUT /users/me` 保存且刷新可见；出售订单页不再出现“确认已交付”，仅说明卖家确认可交易后等待买家确认自提；类目二级新增、启停、刷新持久化、真实商品数和安全删除均通过。临时类目与临时商品已从生产库清理，复核数量均为 0。
...
- 2026-07-16：修复抽象梗 catalog seed 与真实商品自增 ID 冲突：16 件梗商品及图片迁移到 `60001`–`60016`；`50073`–`50075` 按创建审计恢复卖家并保护性下架，`50076`–`50088` 的错误 seed 副本标记删除，后续 repeatable seed 不再覆盖真实商品区间。
...
- 2026-07-16：闭合四条课堂录制真实链路：商品编辑读取所有者详情并真实保存/下架；卖家订单移除越权的“确认已交付”并保持卖家确认可交易、买家确认自提；个人资料只编辑可持久化的昵称/头像并只读展示真实认证字段；后台类目新增 V6 两级结构、启停、实时商品数和完整 CRUD。
