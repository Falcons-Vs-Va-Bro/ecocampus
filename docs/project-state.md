...
- 当前真实页面为 `/`、`/login`、`/favorites`、`/messages`、`/messages/:conversationId`、`/items` 及分类页、`/publish`、`/items/mine`、`/items/:id/edit`、`/profile` 和 `/verify`。
...
- 除 `/`、`/login`、`/favorites`、私信、商品分类、发布/我的商品、个人信息和校园核验外，订单、求购、商品详情等前端路由仍是占位页；公开/用户占位页已统一到市场端彩绘壳，真实业务 UI 仍待接入，后端 API 和前端 wrapper 已经更完整。
