...
- 2026-07-16 Runner 已通过 MySQL 测试库准备、33 项测试和生产 JAR 构建，但 LaunchAgent 部署健康检查失败；工作流已增加不输出环境变量的服务状态、PID、8080 和本机 health 摘要用于恢复诊断。
...
- 2026-07-16：后端部署失败时在 Actions 摘要记录脱敏的 LaunchAgent、进程、端口和本机健康状态，便于在受限 SSH 不提供 Shell 的前提下定位 Mac mini 服务故障。
