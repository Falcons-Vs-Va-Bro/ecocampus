...
- 当前后端认证实现是账号密码：账号必须以 `2292024` 开头，首次登录自动创建 `UNVERIFIED` 用户并保存 BCrypt 哈希，已有账号必须使用原密码；完成校园核验后才变为 `VERIFIED`。
...
- 除 `/`、`/login`、商品浏览/详情、收藏、私信、订单、发布/我的商品、个人信息、校园核验、商品审核、商品治理、用户与黑名单和类目管理外，求购等前端路由仍是占位页；公开/用户占位页已统一到市场端彩绘壳，真实业务 UI 仍待接入，后端 API 和前端 wrapper 已经更完整。
