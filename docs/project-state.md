...
- 课堂单实例生产基线为 Hikari 最大 12/最小空闲 3、连接等待 10 秒，Tomcat 最大线程 100、最小空闲线程 8、最大连接 300、等待队列 100；用于吸收 50+ 人同时访问的短时突发，MySQL 仍保持 151 条全局连接上限。
...
- 页面组件由 `frontend/src/app/routeComponents.ts` 统一按路由懒加载，公开首页不会预加载后台和其他业务页面；`App.tsx` 提供路由加载回退。
- 位图资产已统一为 WebP；商品列表仅前 4 张首屏图片主动加载，其余商品图、侧栏和空状态插画懒加载并异步解码。
...
- 2026-07-14 修正公共首页硬编码登录身份：新增真实 `/auth/me` 用户快捷入口、客户端退出、生产会话 v2 迁移和有效路由守卫；前端 `pnpm lint`、`pnpm build` 通过，后端 32 tests 全通过并完成生产 JAR 构建。GitHub Pages 发布成功，本机 LaunchAgent 已重启生产 JAR；线上产物确认包含“登录 / 注册”和 `ecocampus.auth.real.v2`，匿名 `/auth/me` 返回 401，公网登录、`/auth/me` 与会话列表返回 200。
- 2026-07-14 路由拆包、WebP/懒加载和课堂并发配置变更后运行 `cd frontend && pnpm lint && pnpm build`、`cd backend && ./mvnw test` 和生产 JAR 构建；前端入口从约 1,232 kB（gzip 约 380 kB）降至约 628 kB（gzip 约 206 kB），后端 32 tests 全通过。
- 2026-07-14 上述版本已发布到 GitHub Pages 并重启 Mac mini 后端 LaunchAgent；线上入口仅预加载入口/CSS/JSX runtime，路由 chunk 按需加载，WebP 资产返回 `image/webp`。本机真实商品列表接口完成 600 请求、60 并发压测：0 失败、439.30 req/s、P95 269 ms、最长 477 ms；MySQL `Max_used_connections` 从 5 升至 10/151，慢查询为 0，公网健康检查返回 UP。
...
- 2026-07-14：前端页面改为路由级动态导入，统一 WebP 图片并限制首屏主动加载资源；Mac mini 单实例生产连接池调整为 12、Tomcat 最大线程调整为 100，面向课堂 50+ 人短时并发。
