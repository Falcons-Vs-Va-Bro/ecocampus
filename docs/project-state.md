...
- 2026-07-16 用户商品真实发布链路接线后 `cd frontend && pnpm lint && pnpm build` 通过；生产端用学生账号上传并创建商品 `50073`，学生“我的发布”显示审核中，管理员审核页待审数从 3 增至 4 且出现同一商品；网页批准后待审数恢复 3，卖家摘要与公开详情均返回 `ON_SALE`，浏览器控制台 0 error。
