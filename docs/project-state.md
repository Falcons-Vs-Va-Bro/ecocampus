...
- 当前真实页面为 `/`、`/login`、`/favorites`、`/messages` 和 `/messages/:conversationId`。
...
- 除 `/`、`/login`、`/favorites`、`/messages` 和 `/messages/:conversationId` 外，大多数前端路由仍是占位页；公开/用户占位页已统一到市场端彩绘壳，真实业务 UI 仍待接入，后端 API 和前端 wrapper 已经更完整。
