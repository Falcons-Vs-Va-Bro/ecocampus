...
- 个人中心、校园核验、发布闲置、我的发布和编辑商品已通过 `UnifiedMarketplacePage` 直接复用首页的 `MarketplaceShell`；`frontend/src/styles/marketplace-consistency.css` 只负责收拢这些页面保留的业务内容 DOM，顶部栏、左侧图标/校园插画、选中态和入场动画均由共享壳统一提供。
...
- 除 `/`、`/login`、商品浏览/详情、收藏、私信、订单、求购、发布/我的商品、个人信息、校园核验、商品审核和商品治理外，其余规划中的前端路由仍是占位页；公开/用户占位页已统一到市场端彩绘壳，后端 API 和前端 wrapper 已经更完整。
